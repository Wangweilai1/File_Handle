...
class BatchProcessCSV:
    ...
    def doBatchAction(self):
        ...
        print("处理完成，耗时：%f秒" %(endTime-startTime).seconds)
  ...
  #解决csv浏览乱码问题
        ...
        for row in reader:
            ...
            if lenth>10:
                ...
                writer.writerow([row[column.index("Rank")],row[column.index("Category")],row[column.index("Country")],row[column.index("App Name")],
                                 row[column.index("Value")],row[column.index("Unit")],row[column.index("App Release Date")],row[column.index("Publisher Name")],
                                 row[column.index("Company Name")],row[column.index("Parent Company Name")]])
...
if __name__=="__main__":
    ...
    csvProcess=BatchProcessCSV("c:\\input\\","d:\\output\\")
